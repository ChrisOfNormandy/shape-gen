...
export default function ShapeFlipVerticalForm({ shape }: Readonly<{ shape: IShape }>) {

    const { setShapes } = useShapes();

    const [error, submitAction, isPending] = useActionState<Error | null, FormData>(
        () => {
            const copy = shape.flipVertical();
    ...
    return <form action={submitAction} className='shape-mod-controls-form'>
        ...
        <Button type='submit' disabled={isPending} theme={new ThemeOptions({ background: { style: 'trinary' }, border: { style: 'trinary' } })}>Flip Vertical</Button>
    </form>
}